...
def create_app():
    ...
    @app.route('/')
    def index():
        authorize_url = genomelink.OAuth.authorize_url(scope=['report:eye-color report:beard-thickness report:morning-person report:alcohol-drinking-behavior report:milk-allergy'])

        # Fetching a protected resource using an OAuth2 token if exists.
        reports = []
        if session.get('oauth_token'):
            for name in ['eye-color', 'beard-thickness', 'morning-person', 'alcohol-drinking-behavior', 'milk-allergy']:
